...
class SurveyTable(tables.Table):
    ...
    def render_slug(self, record, value):
        usage_url = reverse("djf_surveys:gracen_admin_survey_details", args=[record.pk])
        edit_url = reverse("djf_surveys:gracen_admin_edit_survey", args=[record.slug])
        delete_url = reverse("djf_surveys:gracen_admin_delete_survey", args=[record.slug])
        version = ""
        if record.gracen_version:
            version = f"""<span class="tooltiptext" style="min-width: 160px;">Gracen Version: {str(record.gracen_version)}</span>"""
        tiles = f"""
